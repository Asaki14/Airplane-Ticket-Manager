...
| 真实接入优先走官方或合作授权路径，浏览器辅助采集只作为受控补充方案 | 降低封禁、合规和长期维护风险 | — Pending |
| v2.0 先跑通单一真实来源和完整用户闭环，再扩展多来源聚合 | 先验证接入架构与真实体验，而不是过早追求覆盖面 | — Pending |
| 当前台展示缓存或部分失败数据时，必须明确提示 freshness 和风险级别 | 真实票价易波动，信任建立依赖透明度 | — Pending |
